...
const Cursor = () => {
    ...
    useEffect(() => {
        ...
        const updateMouse = () => {
            ...
            displayedMouse.current.x +=
                (realMouse.current.x - displayedMouse.current.x) * 0.065;
            displayedMouse.current.y +=
                (realMouse.current.y - displayedMouse.current.y) * 0.065;
    ...
    return (
        <div
            ref={cursorRef}
            className="w-8 h-8 bg-transparent border-[1px] border-textColor rounded-full fixed z-[9999] -translate-x-1/2 -translate-y-1/2 pointer-events-none hidden transition duration-75"
        ></div>
    );
};
